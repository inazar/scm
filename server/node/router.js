define([
	...
	function fireRoute(params, user, path){
	...
	var RouterBase = declare(null, {
		...
		register: function(/*String|RegExp*/ route, /*Function*/ callback){
			...
			//		- path: The new hash being shifted to
		...
		_getParameters: function (routeObj, result) {
			var params = {}, j, lj;

			if(routeObj.parameterNames) {
				var parameterNames = routeObj.parameterNames;
				for(j=0, lj=parameterNames.length; j<lj; ++j){
					params[parameterNames[j]] = result[j+1];
				}
			} else params = result.slice(1);
			return params;
		},
